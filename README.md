...
**Blender Overview:**  
The Blender script is designed for **Blender 3.6.0** and allows users to visualize `.xyz` and `.ply` files generated by the Python script. It imports `.xyz` point clouds as cubes, applies Convex Hulls to `.ply` meshes, organizes objects into collections (`XYZ`, `PLY`, `CH`), and supports user selection of a number of objects to display.
...
2. **Blender add-on/script** – imports `.xyz` and `.ply` files into Blender, visualizes point clouds as cubes, applies Convex Hulls to meshes, and organizes objects into collections.
